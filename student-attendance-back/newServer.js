...
app.post('/signup', async (req, res) => {
    const { student_id, password } = req.body;  // email을 student_id로 변경

    try {
        // 사용자 정보를 데이터베이스에 삽입 (비밀번호 해싱 없이)
        const [result] = await dbPool.execute(
            'INSERT INTO students (student_id, password) VALUES (?, ?)', 
            [student_id, password]  // student_id로 삽입
        );
...
app.post('/login', async (req, res) => {
    ...
    try {
        // 학번으로 사용자 검색
        const [rows] = await dbPool.execute('SELECT * FROM students WHERE student_id = ?', [student_id]);
